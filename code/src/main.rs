...
fn main() {
    ...
    while window.is_open() && !window.is_key_down(Key::Escape) {
        ...
        for i in 0..particles.pos.len() {
            draw_square(
                &mut buffer,
                particles.pos[i].x as i32,
                HEIGHT as i32 - particles.pos[i].y as i32,
                15,
            );
        }
...
fn main() {
    eprintln!(
        "The window renderer is disabled. Run the terminal renderer with:\n\
